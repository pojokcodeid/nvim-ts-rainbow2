...
local function find_container(matches, container, level)
	for _, match in matches:iter() do
		if match.container == container then
			return match, level
		end
...
function M.on_attach(bufnr, settings)
	...
	match_trees[bufnr] = {}
	-- Menggunakan 'trees()' untuk menggantikan 'for_each_tree'
	for _, tree in ipairs(parser:trees()) do
		local sub_lang = tree:lang()
		local changes = {
			{ tree:root():range() },
		}
